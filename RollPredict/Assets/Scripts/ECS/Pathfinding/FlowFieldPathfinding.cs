...
namespace Frame.ECS
{
    ...
    public static class FlowFieldPathfinding
    {
        ...
        {
            /// <summary>
            /// 距离场：从目标点到每个格子的距离（Fix64.MaxValue表示不可达）
            /// </summary>
            public Dictionary<GridNode, Fix64> distanceField;
            
            /// <summary>
            /// 梯度场：每个格子指向最近目标的方向向量（归一化）
            /// </summary>
        ...
        public static FlowFieldData? ComputeFlowField(GridMapComponent map, List<FixVector2> targets)
        {
        ...
        public static FixVector2 GetDirection(FlowFieldData flowField, GridMapComponent map, FixVector2 currentPos)
        {
            GridNode currentNode = map.WorldToGrid(currentPos);
            
            // 查询梯度场
            if (flowField.gradientField.TryGetValue(currentNode, out var direction))
            {
                return direction;
            }
