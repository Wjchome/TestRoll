...
//         /// <summary>
//         /// 执行查询逻辑（优化版本）
//         /// 如果只有 WithAll 条件，从最小的组件集合开始做交集，提高性能
//         /// </summary>
//         private List<Entity> ExecuteQuery()
//         {
//             // 优化：如果只有 WithAll 条件，使用交集算法
//             if (_allTypes.Count > 0 && _anyTypes.Count == 0 && _noneTypes.Count == 0)
//             {
//                 return ExecuteWithAllOptimized();
//             }
//             
//             // 否则使用通用算法（遍历所有 Entity）
...
//         
//         /// <summary>
//         /// 优化的 WithAll 查询：从最小的组件集合开始做交集
//         /// 复杂度：O(m * k)，其中 m 是最小组件集合的大小，k 是其他组件类型的数量
//         /// 通常 m << n（所有 Entity 数量），所以性能更好
//         /// </summary>
//         private List<Entity> ExecuteWithAllOptimized()
//         {
...
//             {
//                 var storage = _world.GetComponentStorage(type);
//                 if (storage == null)
//                 {
//                     // 如果某个组件类型不存在，直接返回空列表
//                     return new List<Entity>();
...
//             var smallestStorage = _world.GetComponentStorage(smallestType);
//             foreach (var entity in smallestStorage.GetAllEntities())
//             {
//                 candidateEntities.Add(entity);
//             }
//             
//             // 对每个候选 Entity，检查是否拥有所有其他组件
//             var result = new List<Entity>();
...
//                 foreach (var type in _allTypes)
//                 {
//                     if (type != smallestType && !_world.HasComponentOfType(entity, type))
//                     {
//                         hasAll = false;
//                         break;
//                     }
